...
const settings = definePluginSettings({
    showChatBarButton: {
        ...
        description: "Show the SmoothType button in the chat bar",
        default: true,
    },
    animationType: {
    ...
    transitionDelay: {
        type: OptionType.COMPONENT,
        description: "Caret move duration (ms)",
        default: 55,
        component: () => (
            <div>
                <Forms.FormTitle tag="h5">Caret movement speed</Forms.FormTitle>
                <Forms.FormText style={{ marginBottom: 8 }}>Transition duration (ms) — higher is slower and smoother.</Forms.FormText>
                <Slider
                    initialValue={settings.store.transitionDelay ?? 55}
                    onValueChange={(v: number) => { settings.store.transitionDelay = v; applyCSS(); }}
                    minValue={0}
                    maxValue={800}
        ...
        component: () => (
            <div>
                ...
                <Slider
                    ...
                    onValueChange={(v: number) => { settings.store.caretWidth = v; applyCSS(); }}
                    minValue={1}
                    maxValue={10}
                    markers={[1, 2, 4, 6, 10]}
                    stickToMarkers={false}
                ...
                <Slider
                    ...
                    onValueChange={(v: number) => { settings.store.caretOpacity = clamp(v / 100, 0.25, 1); applyCSS(); }}
                    minValue={25}
                    maxValue={100}
                ...
                <Slider
                    ...
                    onValueChange={(v: number) => { settings.store.caretGlow = v; applyCSS(); }}
                    minValue={0}
                    maxValue={24}
                    markers={[0, 4, 8, 16, 24]}
                    stickToMarkers={false}
    ...
    caretGlow: {
        type: OptionType.HIDDEN,
        description: "Soft glow radius (px)",
        default: 0,
    },
    presets: {
        type: OptionType.COMPONENT,
...
const SmoothTypeIcon: IconComponent = ({ height = 20, width = 20, className }) => (
    <svg aria-hidden width={width} height={height} className={className} viewBox="0 0 24 24" fill="currentColor">
    ...
    return (
        <div className={Margins.bottom8}>
            <Forms.FormText style={{ marginBottom: 12 }}>
                Quick controls from the chat bar.
            </Forms.FormText>

            <Forms.FormTitle tag="h5">Caret color</Forms.FormTitle>
            <ColorPicker
                color={settings.store.caretColor}
                onChange={c => { settings.store.caretColor = c; applyCSS(); }}
                showEyeDropper={true}
            />
            ...
            <SearchableSelect
                ...
                maxVisibleItems={6}
                closeOnSelect
                onChange={(v: string) => { settings.store.animationType = v; applyCSS(); }}
            />
            ...
            <div style={{ marginTop: 20 }}>
                <Button color={Button.Colors.PRIMARY} onClick={close}>Done</Button>
            </div>
        </div>
    );
}
...
function openSmoothTypeHud() {
    const key = openModal(props => (
        <ModalRoot {...props} size={ModalSize.MEDIUM}>
            <ModalHeader>
                <Text variant="heading-lg/semibold" style={{ flexGrow: 1 }}>SmoothType</Text>
...
const SmoothTypeChatBarButton: ChatBarButtonFactory = ({ isAnyChat }) => {
    ...
    return (
        <ChatBarButton tooltip="SmoothType" onClick={() => openSmoothTypeHud()} buttonProps={{ "aria-haspopup": "dialog" }}>
            <SmoothTypeIcon />
        </ChatBarButton>
    );
};
...
export default definePlugin({
    name: "SmoothType",
    description: "Replaces the default Discord cursor with a smooth animated caret. Customize color, speed, glow and more.",
