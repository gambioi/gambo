...
export const MicrophoneSettingsModal = (props: MicrophoneSettingsModalProps) => {
    ...
    const settingsCardForceBitrate =
        <SettingsModalCard
            title="Force 320kbps"
            flex={0.2}
            switchEnabled
            switchProps={{
                checked: forceBitrate ?? false,
                disabled: isSaving,
                onChange: status => setForceBitrate(status)
            }}>
            <SettingsModalCardItem>
                <Forms.FormText style={{ opacity: 0.6, fontSize: "0.75em" }}>
                    Force le bitrate max peu importe le salon
                </Forms.FormText>
            </SettingsModalCardItem>
        </SettingsModalCard>;
    ...
    const settingsCardAgcBoost =
        <SettingsModalCard
            title="Mic Boost (AGC)"
            flex={0.2}
            switchEnabled
            switchProps={{
                checked: agcBoostEnabled ?? false,
                disabled: isSaving,
                onChange: status => { setAgcBoostEnabled(status); setAgcBoost(true); }
            }}>
            <SettingsModalCardItem>
                <Forms.FormText style={{ opacity: 0.6, fontSize: "0.75em" }}>
                    Amplifie le signal avant l'encodage
                </Forms.FormText>
            </SettingsModalCardItem>
        </SettingsModalCard>;

    const boostLevelLabels = ["Off", "1 – AGC", "2 – AGC+Vol", "3 – AGC+Vol+NS off", "4 – MAX"];
    const settingsCardBoostLevel =
        <SettingsModalCard
            title="Boost Level"
            switchEnabled
            flex={0.5}
            switchProps={{
                checked: boostLevelEnabled ?? false,
                disabled: !agcBoostEnabled || isSaving,
                onChange: status => setBoostLevelEnabled(status)
            }}>
            <SettingsModalCardItem>
                <div style={{ paddingTop: "0.3em", paddingRight: "0.4em", paddingLeft: "0.4em", boxSizing: "border-box" }}>
                    <Slider
                        disabled={!boostLevelEnabled || !agcBoostEnabled || isSaving}
                        onValueChange={value => setBoostLevel(Math.round(value))}
                        initialValue={boostLevel ?? 1}
                        minValue={0} maxValue={4}
                        markers={[0, 1, 2, 3, 4]}
                        onValueRender={value => boostLevelLabels[Math.round(value)] ?? `${Math.round(value)}`} />
                </div>
            </SettingsModalCardItem>
            <SettingsModalCardItem>
                <Forms.FormText style={{ opacity: 0.6, fontSize: "0.75em" }}>
                    Niveau 3-4 : désactive noise suppression / echo cancel
                </Forms.FormText>
            </SettingsModalCardItem>
        </SettingsModalCard>;
    ...
    const settingsCardNoiseSuppressionOff =
        <SettingsModalCard
            title="NS Off"
            flex={0.2}
            switchEnabled
            switchProps={{
                checked: noiseSuppressionOff ?? false,
                disabled: isSaving,
                onChange: status => setNoiseSuppressionOff(status)
            }}>
            <SettingsModalCardItem>
                <Forms.FormText style={{ opacity: 0.6, fontSize: "0.75em" }}>
    ...
    const settingsCardEchoCancellationOff =
        <SettingsModalCard
            title="EC Off"
            flex={0.2}
            switchEnabled
            switchProps={{
                checked: echoCancellationOff ?? false,
                disabled: isSaving,
                onChange: status => setEchoCancellationOff(status)
            }}>
            <SettingsModalCardItem>
                <Forms.FormText style={{ opacity: 0.6, fontSize: "0.75em" }}>
                    Désactive l'annulation d'écho
                </Forms.FormText>
            </SettingsModalCardItem>
        </SettingsModalCard>;

    const settingsCardOpusAudioMode =
        <SettingsModalCard
            title="Opus Audio"
            flex={0.2}
            switchEnabled
            switchProps={{
                checked: opusAudioMode ?? false,
                disabled: isSaving,
                onChange: status => setOpusAudioMode(status)
            }}>
            <SettingsModalCardItem>
                <Forms.FormText style={{ opacity: 0.6, fontSize: "0.75em" }}>
                    Mode audio large bande (vs voip)
                </Forms.FormText>
            </SettingsModalCardItem>
        </SettingsModalCard>;
    ...
    const sectionLabel = (text: string) => (
        <Forms.FormTitle tag="h5" style={{ margin: "0.2em 0 0 0.1em", opacity: 0.5, fontSize: "0.7em", textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {text}
        </Forms.FormTitle>
    );
    ...
    return (
        ...
        >
            {simpleMode
                ? <div style={{ width: "48em", display: "flex", flexDirection: "column", gap: "0.6em" }}>
                    {sectionLabel("Qualité audio")}
                    <SettingsModalCardRow>
                        {settingsCardForceBitrate}
                        {settingsCardVoiceBitrateSimple}
                        {settingsCardChannelsSimple}
                    </SettingsModalCardRow>
                    {sectionLabel("Traitement audio")}
                    <SettingsModalCardRow>
                        {settingsCardNoiseSuppressionOff}
                        {settingsCardEchoCancellationOff}
                        {settingsCardOpusAudioMode}
                    </SettingsModalCardRow>
                    {sectionLabel("Boost micro")}
                    <SettingsModalCardRow>
                ...
                : <div style={{ display: "flex", flexDirection: "column", width: "52em", gap: "0.6em", maxHeight: "40em" }}>
                    {sectionLabel("Qualité audio")}
                    <SettingsModalCardRow>
                        {settingsCardForceBitrate}
