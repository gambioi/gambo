...
// Only import pure modules that don't import other parts of Gambcord.
...
const setCssDebounced = debounce((css: string) => GambcordNative.quickCss.set(css));

const themeStore = DataStore.createStore("GambcordThemes", "GambcordThemeData");

// probably should make this less cursed at some point
window.GambcordNative = {
    themes: {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Vendicated/Gambcord" }),
    ...
    quickCss: {
        get: () => DataStore.get("GambcordQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("GambcordQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "GambcordQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => GambcordNative.quickCss.get();
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("GambcordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("GambcordSettings", JSON.stringify(s)),
