...
// Only import pure modules that don't import other parts of Gambo.
...
const setCssDebounced = debounce((css: string) => GamboNative.quickCss.set(css));

const themeStore = DataStore.createStore("GamboThemes", "GamboThemeData");
...
window.GamboNative = {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Vendicated/Gambo" }),
    ...
    quickCss: {
        get: () => DataStore.get("GamboQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("GamboQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "GamboQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => GamboNative.quickCss.get();
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("GamboSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("GamboSettings", JSON.stringify(s)),
