...
While it was primarily created for Gambcord, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @gambcord/discord-types
yarn add -D @gambcord/discord-types
pnpm add -D @gambcord/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@gambcord/discord-types";
...
```ts
import { ApplicationCommandType } from "@gambcord/discord-types/enums";
